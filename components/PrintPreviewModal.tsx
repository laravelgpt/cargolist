...
const PrintPreviewModal: React.FC<PrintPreviewModalProps> = ({ onClose, themeState, ...props }) => {
    ...
    // This effect populates the iframe with a static version of the document
    useEffect(() => {
        ...
        // The full HTML structure for the iframe's content
        doc.open();
        doc.write(`
            <!DOCTYPE html>
            <html lang="en">
    ...
    return (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50 no-print" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div className="flex flex-col bg-gray-200 rounded-lg shadow-xl w-[95vw] h-[95vh] max-w-7xl">
                {/* Modal Header */}
                <div className="flex justify-between items-center p-4 bg-white border-b border-gray-300 rounded-t-lg">
                ...
                <div className="p-4 flex-grow bg-gray-200 overflow-y-auto">
                    <iframe ref={iframeRef} title="Printable Document" className="w-full h-full border-0 shadow-lg" />
                </div>
                
                {/* Modal Footer */}
                <div className="flex items-center justify-end p-4 space-x-2 bg-white border-t border-gray-200 rounded-b-lg">
